            ...
            <tr data-id="${employee.id}">
                ...
                <td>
                    <div class="table-actions">
                        ...
                        ${canDelete() ? '<button class="btn btn-sm btn-ghost btn-delete" title="Eliminar"><svg class="svg-icon" viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg></button>' : ''}
                    </div>
                </td>
            </tr>
        `;
...
async function saveEmployee() {
  ...
  if (parseFloat(employeeData.salary) < 0) {
    showToast("El salario no puede ser negativo", "error");
    return;
  }
  ...
  // Validación de Teléfono (números y guiones, min 7 chars)
  const phoneCheck = employeeData.phone.replace(/[^0-9]/g, '');
  if (employeeData.phone && phoneCheck.length < 7) {
    showToast("El número de teléfono parece incompleto", "error");
    return;
  }

  // Validación de Fecha de Contratación (No futuro)
  if (employeeData.hire_date) {
    const hireDate = new Date(employeeData.hire_date);
    const today = new Date();
    // Resetear horas para comparar solo fechas
    today.setHours(0, 0, 0, 0);
    hireDate.setHours(0, 0, 0, 0); // Ajuste zona horaria puede afectar, usar string comparison o UTC

    if (hireDate > today) {
      showToast("La fecha de contratación no puede ser futura", "error");
      return;
    }
  }
