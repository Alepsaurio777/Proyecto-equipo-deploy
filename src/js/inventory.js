...
async function loadProducts(status = 'active') {
    try {
        const response = await fetch(`http://localhost/Proyecto-de-Equipo/api/products.php?status=${status}`);
...
async function loadCategories() {
    try {
        const response = await fetch('http://localhost/Proyecto-de-Equipo/api/products.php');
        const data = await response.json();
        if (data.success) {
...
async function deleteProduct(productId) {
    try {
        const response = await fetch(`http://localhost/Proyecto-de-Equipo/api/products.php?id=${productId}`, { method: 'DELETE' });
        const data = await response.json();
        if (data.success) {
            await Promise.all([loadProducts('active'), loadProducts('inactive')]);
            return true;
